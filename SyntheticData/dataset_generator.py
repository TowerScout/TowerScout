...
def create_image_anno(objects, distractor_objects, img_file, anno_file, bg_file,  w=WIDTH, h=HEIGHT, scale_augment=False, rotation_augment=False, blending_list=['none'], dontocclude=False):
    ...
    while True:
        ...
        for idx, obj in enumerate(all_objects):
           ...
           foreground = foreground.crop((xmin-50, ymin-50, xmax+50, ymax+50))
           orig_w, orig_h = foreground.size
           mask_file =  get_mask_file(obj[0])
           mask = Image.open(mask_file)
           #mask = mask.crop((xmin, ymin, xmax, ymax))
           mask = mask.crop((xmin-50, ymin-50, xmax+50, ymax+50))
           if INVERTED_MASK:
           ...
           for i in range(len(blending_list)):
               if blending_list[i] == 'none' or blending_list[i] == 'motion':
                  ...
                  #Jia - Blurring
                  #foreground_o = Image.open(obj[0])
                  #foreground_o = foreground_o.crop((xmin-50, ymin-50, xmax+50, ymax+50))
                  #mask_o = Image.open(mask_file)
                  #mask_o = mask_o.crop((xmin-50, ymin-50, xmax+50, ymax+50))

                  mask_blur = mask.filter(ImageFilter.GaussianBlur(10))
                  backgrounds[i].paste(foreground, (x, y), #Image.fromarray(cv2.GaussianBlur(PIL2array1C(mask),(45,45),0)))
                  mask_blur)
